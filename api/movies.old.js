...
const listMovies = (db, where) => {
  let movies, comments;

  return db.allAsync('SELECT * FROM movies' + (where ? ' ' + where : '') + ' ORDER BY place')
    .then(rows => {
      movies = rows;
      return db.allAsync('SELECT * from comments WHERE id in (' + movies.map(m => m.id).join(', ') + ') ORDER BY created');
    })
    .then(rows => {
      comments = rows;

      movies.forEach(m => m.comments = comments.filter(c => c.movieid === m.id));
...
const list = (req, res, next) => {
  listMovies(req.db).then(r => console.log(JSON.stringify(r)));
